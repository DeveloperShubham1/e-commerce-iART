...
export const updateMerchant = async (req, res) => {
    try {
        ...
        if (siteBaseUrl !== undefined)
            updateFields["instagram.siteBaseUrl"] = siteBaseUrl;
...
export const getDashboardData = async (req, res) => {
    try {
        ...
        const [
            ...
            Merchant.countDocuments({ isSubscribed: false }),
            Merchant.find({})
                .select({
                    MerchantName: 1,
                    OwnerName: 1,
...
export const getOrdersList = async (req, res) => {
    try {
        ...
        const filter = {};
        if (search) {
            const searchFilter = [{ orderId: { $regex: search, $options: "i" } }];
...
export const toggleMerchantSubscription = async (req, res) => {
    try {
        ...
        if (!id)
            return res
                .status(400)
                .json({ success: false, message: "Merchant id required" });
        ...
        return res.status(200).json({
            ...
            merchant: { _id: merchant._id, isSubscribed: merchant.isSubscribed },
        });
    } catch (error) {
...
export const getSalesSummary = async (req, res) => {
    try {
        ...
        const totalCustomers =
            (customersAgg[0] && customersAgg[0].uniqueCustomers) || 0;
        ...
        const salesAgg = await Order.aggregate([
            { $match: { $or: [{ isPaid: true }, { paymentStatus: "paid" }] } },
            {
                $group: {
                    _id: null,
                    totalAmountPaid: {
                        $sum: { $ifNull: ["$amountPaid", "$totalAmount"] },
                    },
                },
            },
        ]);
        ...
        return res
            .status(500)
            .json({ success: false, message: "Failed to fetch sales summary." });
    }
};
    ...
    try {
        ...
        const agg = await Order.aggregate([
            { $match: { merchantId: merchant._id } },
            {
                $group: {
                    _id: "$merchantId",
                    totalSales: { $sum: { $ifNull: ["$amountPaid", "$totalAmount"] } },
                    ordersCount: { $sum: 1 },
                },
            },
        ]);
        ...
        return res
            .status(200)
            .json({ success: true, merchant: { ...merchant, stats } });
    } catch (error) {
        console.error("Get merchant detail error:", error);
        return res
            .status(500)
            .json({ success: false, message: "Failed to fetch merchant detail." });
    }
};
...
export const getMerchantsRevenue = async (req, res) => {
    try {
        ...
        const agg = await Order.aggregate([
            ...
            {
                $project: {
                    merchant: { MerchantName: 1, email: 1, phone: 1 },
                    totalSales: 1,
                    ordersCount: 1,
                },
            },
        ...
        return res
            .status(500)
            .json({ success: false, message: "Failed to fetch merchants revenue." });
    }
};
...
export const getCustomerDetail = async (req, res) => {
    try {
        ...
        const agg = await Order.aggregate([
            { $match: { userId: user._id } },
            {
                $group: {
                    _id: "$userId",
                    totalOrders: { $sum: 1 },
                    totalSpent: { $sum: { $ifNull: ["$amountPaid", "$totalAmount"] } },
                },
            },
        ]);
        ...
        return res
            .status(500)
            .json({ success: false, message: "Failed to fetch customer detail." });
    }
};

// ============================= GET ALL CUSTOMERS WITH THEIR MERCHANTS DETAIL =============================
...
// ============================= GET ALL ORDERS WITH THEIR MERCHANTS AND CUSTOMERS DETAIL =============================
export const getAllOrders = async (req, res) => {
    try {
        ...
        const filteredOrders = includeGuests
            ? orders
            : orders.filter((o) => !o.userId?.isGuest);
